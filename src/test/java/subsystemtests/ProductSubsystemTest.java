...
public class ProductSubsystemTest extends TestCase {
	...
	public void testGetProductNames() {
		...
		ProductSubsystem pss = new ProductSubsystemFacade();
		try {
			List<String> found = pss.getProductList(ProductSubsystemFacade.createCatalog(1, "abc"))
				      .stream()
				      .map(cat -> cat.getProductName())
				      .collect(Collectors.toList());
			boolean valfound = false;
			for(String prodData : found) {
				
					if(prodData.equals(expected)) valfound = true;
