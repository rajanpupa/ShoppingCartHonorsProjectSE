...
public class MaintainCatalogsWindow extends Stage implements MessageableWindow {
	...
	public MaintainCatalogsWindow(Stage primaryStage) {
		...
		// Make sure that mouse click outside of table will also restore row
		// selection
	...
	public void addItem(CatalogPres item){
		try {
			...
			messageBar.setText("Backend Exception");
		}
	...
	private void setUpTable() {
		...
		idCol = TableUtil.makeEditableTableColumn(table, new CatalogPres(),
				"ID", "idProperty", 100);
		...
		nameCol = TableUtil.makeEditableTableColumn(table, new CatalogPres(),
				"Name", "nameProperty", 230);
		nameCol.setOnEditCommit(t -> {
			try{
				...
				TableUtil.refreshTable(table,
						ManageProductsData.INSTANCE.getManageCatalogsSynchronizer());
				ManageProductsData.INSTANCE.editCatalog(instance);
			}catch(BackendException e){
				messageBar.setText("could not save ");
			}
		});
	...
	private HBox setUpButtons() {
		...
		deleteButton.setOnAction(evt -> {
			...
			ObservableList<Integer> selectedIndices = table.getSelectionModel()
					.getSelectedIndices();
			ObservableList<CatalogPres> selectedItems = table
			...
			if (tableItems.isEmpty()) {
				...
				try {
					...
					if (result) {
						//ManageProductsUIControl.INSTANCE.deleteCatalog(selectedItems.get(0).getCatalog());//dont do it here
						table.setItems(ManageProductsData.INSTANCE
								.getCatalogList());
						clearMessages();
		...
		backButton.setOnAction(ManageProductsUIControl.INSTANCE
