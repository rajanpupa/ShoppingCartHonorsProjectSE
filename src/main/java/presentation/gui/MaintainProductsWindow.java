...
public class MaintainProductsWindow extends Stage implements MessageableWindow {
	...
	private void setUpCombo() {
		catalogCombo.valueProperty().addListener(new ChangeListener<String>() {
	        ...
	        public void changed(ObservableValue ov, String oldval, String newval) {
				...
				try {
					temp = ManageProductsData.INSTANCE.getCatalogList()
					                .stream()
	...
	public void addItem(ProductPres item) {
		try {
			CatalogPres selected = ManageProductsData.INSTANCE
					.getSelectedCatalog();
			...
			// Log sth here
			System.out.println("error saving the product");
			messageBar.setText("Failed to save product. Backend error");
		}
	}
	...
	private void setUpTable() {
        ...
        quantityCol.setOnEditCommit(t -> {
		   try{
			   ...
			   ManageProductsData.INSTANCE.editProduct(instance);//update db
			   instance.setQuantityAvail(new SimpleStringProperty(t.getNewValue()));		   
			   TableUtil.refreshTable(table, synchronizer);
		   }catch(BackendException e){
			   messageBar.setText("could not save product");
		   }
	     }); 
		    ...
		    if(tableItems.isEmpty()) {
		    	...
		    	messageBar.setText("Please select a row.");
						...
						boolean result = ManageProductsData.INSTANCE
								.removeFromProductList(selectedCatalog,
										selectedItems);
						if (result) {
							ManageProductsData.INSTANCE.refreshData();
