...
public class AddProductPopup extends Popup {
	...
	TextField manufactureDate = new TextField((new SimpleDateFormat("MM/dd/yyyy")).format(new Date()));
	...
	private HBox setUpButtons() {
		...
		addButton.setOnAction(evt -> {
			...
			if(id.getText().trim().equals("")) {
			...
			else if (description.getText().trim().equals("")){
						messageBar.setText("Description field must be nonempty!");
			} else {
				String idNewVal = id.getText();
				if (idNewVal.equals("0")) {
					idNewVal = DefaultData.generateId(100);
				} // Catalog c, Integer pi, String pn, int qa, double
					// up, LocalDate md, String d
				...
				try {
					Product newProd = ProductSubsystemFacade.createProduct(
							...
							LocalDate.parse(manufactureDate.getText(),DateTimeFormatter.ofPattern("MM/dd/yyyy")),
							description.getText());
					
					ProductPres prodPres = new ProductPres();
					
					// actually save the product in db
					//Integer newId = productSubSystem.saveNewProduct(newProd);
					//newProd.setProductId(newId);
					prodPres.setProduct(newProd);
					//System.out.println("=================The new product id generated is " + newId);
					maintainProductsWindow.addItem(prodPres);
				} catch (BackendException be) {
					// log database error here
					System.err.println("Could not save Catalog due to Database error....");
					be.printStackTrace();
					messageBar.setText("Could not save Product due to Database error");
				}
				messageBar.setText("");
				hide();
			}
