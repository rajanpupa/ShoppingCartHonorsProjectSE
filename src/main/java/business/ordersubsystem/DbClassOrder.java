...
class DbClassOrder implements DbClass, DbClassOrderForTest {
	...
	void submitOrder(ShoppingCart shopCart) throws DatabaseException{
		LOG.info("In Submit order");
		//convert shopping cart items to orderItems
		orderItems = new LinkedList<OrderItem>();
		for(CartItem ci: shopCart.getCartItems())
		{
			String quantity = ci.getQuantity();
			String price = ci.getTotalprice();
			orderItem = new OrderItemImpl(ci.getProductName(), Integer.parseInt(quantity), Double.parseDouble(price));
			// set productid
	...
	private void buildSaveOrderQuery() {
		...
		String date = GuiUtils.localDateAsString(order.getOrderDate());
	...
	private void buildSaveOrderItemQuery() {
		...
		query = "INSERT INTO orderitem( orderid, productid, quantity, totalprice, shipmentcost, taxamount) "
				...
				+","
				+0.1*orderItem.getTotalPrice() 
				+","
				+0.07*orderItem.getTotalPrice()
				+")";
	}
		...
		try {
			while (rs.next()) {
				...
				OrderItem oi = new OrderItemImpl(pd.getProductName(), quantity,
						price);
				oi.setProductId(pd.getProductId());
				orderItems.add(oi);
			}
